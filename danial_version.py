	...
	def train(self, model_signature):
		...
		if self.model_id is 3:
			...
			if self.save_surrogatedata is True:
				with open(('%s/tain_metrics.txt' % (self.path)),'ab') as outfile:
...
class ptReplica(multiprocessing.Process):
	...
	def run(self):
		...
		for i in range(samples-1):
			...
			#print('w ', w[:5])
			if u < mh_prob:
				# print ('#:', i,'dfl', diff_likelihood, 'dfp',diff_prior ,' dfl+p', diff_likelihood  + diff_prior, 'ACCEPTED - m_p: ', mh_prob, '- #ch ' ,self.temperature ,'\n')
				print ('#:', i,'l prop', likelihood_proposal, 'l',likelihood ,' dfl', diff_likelihood, 'ACCEPTED - m_p: ', mh_prob, '- #ch ' ,self.temperature ,'\n')
				naccept  =  naccept + 1
				likelihood = likelihood_proposal
				prior_current = prior_prop
				w = w_proposal
				likeh_list[i+1,1] = likelihood*self.temperature
				...
				# print ('#:', i,'dfl', diff_likelihood, 'dfp',diff_prior ,' dfl+p', diff_likelihood  + diff_prior, 'REJECTED - m_p: ', mh_prob, '- #ch ' ,self.temperature ,'\n')
				print ('#:', i,'l prop', likelihood_proposal, 'l',likelihood ,' dfl', diff_likelihood, 'ACCEPTED - m_p: ', mh_prob, '- #ch ' ,self.temperature ,'\n')
				pos_w[i+1,] = pos_w[i,] 
			...
			# 	new_prop_list = prop_list.copy()
			# 	new_likeh_list = likeh_list.copy()
			# 	surrogate_X, surrogate_Y = new_prop_list[i+1-self.surrogate_interval:i,:],new_likeh_list[i+1-self.surrogate_interval:i,0]
			# 	surrogate_Y = surrogate_Y.reshape(surrogate_Y.shape[0],1)
			# 	param = np.concatenate([surrogate_X.copy(), surrogate_Y.copy()],axis=1)
		...
		file_name = self.path+'/posterior/pos_w/'+'chain_ws'+ str(self.temperature)+ '.txt'
...
#problem ={ i
	...
	problemfolder = '/home/danial/Desktop/SurrogatePT/SurrogateResults_/'  # change this to your directory for results output - produces large datasets
...
# Plots
	...
	likelihood_accpt = likelihood_rep[:,1] # just plot proposed likelihood_accpt
	likelihood_accpt = np.asarray(np.split(likelihood_accpt, num_chains))

# Plots
	plt.plot(likelihood_accpt.T)


	plt.xlabel('Samples', fontsize=18)
	plt.ylabel('Likelihood_accpt', fontsize=16)

	plt.legend(loc='upper left')
	plt.savefig(path+'/likelihood_accpt.png') 
	plt.savefig(path_db+'/likelihood_accpt.png')
	plt.clf()
